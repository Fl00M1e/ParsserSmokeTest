...
## 2026-09-16 — дедупликация и серые Analyze

- Общий модуль `dedup.py` нормализует ссылки и email для чтения и записи.
- Перед вставкой перечитывается целевой лист; дубли и повторные строки без email отбрасываются, новые контакты известного профиля сохраняются.
- Запись идёт ниже занятых строк и проверяется по точному диапазону. При ошибке бот останавливается без повторной вставки.
- Пустой лист поддерживается; буфер обмена восстанавливается, имена вставляются как текст.
- Поиск учитывает зелёные и серые Analyze, ссылки на готовый отчёт и вложенные неактивные кнопки. Идентификатор карточки сохраняется при смене Analyze на Analyzed.
- Добавлены тесты дедупликации, записи и реальных DOM-переходов в изолированном Chrome.
- Windows-сборка использует общий launcher и поддерживает self-test с записью лога.

Ниже — история прежних изменений. Актуальное поведение: проверяется только целевой лист, кэш живёт до закрытия/обновления, лист перечитывается перед вставкой. Описания загрузки всех листов и очистки после каждого запуска ниже относятся к старой реализации.
